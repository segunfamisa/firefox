...
/**
 * Fenix equivalent of [mozilla.components.feature.importer.ImporterResult]
 *
 ...
 *
 * Main difference here is [FenixImporterResult.Failure] which maps
 ...
 */
sealed interface FenixImporterResult {
    /**
     * The import succeeded.
     *
     * @property importCount The number of items imported.
     */
    data class Success(val importCount: Int) : FenixImporterResult

    /** The import failed due to an error. */
    data class Failure(val error: FenixBookmarkImporterError) : FenixImporterResult

    /** The user cancelled the import. */
    data object Canceled : FenixImporterResult
}
