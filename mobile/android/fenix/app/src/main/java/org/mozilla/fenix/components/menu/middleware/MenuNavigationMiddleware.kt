...
class MenuNavigationMiddleware(
    ...
    override fun invoke(
        ...
        scope.launch {
            when (action) {
                ...
                is MenuAction.Navigate.Passwords ->
                    navController.nav(
                        R.id.menuDialogFragment,
                ...
                is MenuAction.Navigate.DiscoverMoreExtensions ->
                    store.openToBrowser(BrowserNavigationParams(url = AMO_HOMEPAGE_FOR_ANDROID))
                ...
                is MenuAction.Navigate.WebCompatReporter -> {
                    ...
                    session?.content?.url?.let { tabUrl ->
                        if (settings.isTelemetryEnabled) {
                            ...
                            store.openToBrowser(BrowserNavigationParams(url = "$WEB_COMPAT_REPORTER_URL$tabUrl"))
                        }
                    }
                }
    ...
    private fun Store<MenuState, MenuAction>.dismiss() {
        (this as? MenuStore)?.emitEffect(MenuEffect.Dismiss)
    }

    private fun Store<MenuState, MenuAction>.openToBrowser(params: BrowserNavigationParams) {
        (this as? MenuStore)?.emitEffect(MenuEffect.OpenToBrowser(params))
    }
