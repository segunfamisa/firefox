 ...
 *
 * `clock` is injectable for tests; production callers use the default `Date()`.
 */
internal fun generateFennecVersionCode(abi: String, clock: () -> Date = ::Date): Int {
    ...
    val base = fennecBaseVersionCode(clock())
...
private fun fennecBaseVersionCode(build: Date): Int {
    val format = SimpleDateFormat("yyyyMMddHHmmss", Locale.US)
    val cutoff = format.parse("20141228000000")
    return Math.floor((build.time - cutoff.time) / (1000.0 * 60.0 * 60.0)).toInt()
}
