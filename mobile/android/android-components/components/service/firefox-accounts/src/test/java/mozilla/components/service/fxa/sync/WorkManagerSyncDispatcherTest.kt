...
class WorkManagerSyncDispatcherTest {
    ...
    /**
     * Test rule that allows the live data observer in [WorkersLiveDataObserver] to execute
     * synchronously
     */
    @get:Rule
    ...
    @Test
    fun `when using native sync status, after a backed off immediate sync, the observers go back to idle state`() =
        runTest(testCoroutineDispatcher) {
            // given a sync dispatcher configured to use native sync status
            val syncDispatcher = makeSyncDispatcher(
                syncConfig = syncConfig.copy(useNativeSyncStatus = true),
            )
    ...
    @Test
    fun `when using native sync status, after a backed off periodic sync result, the observers go back to idle state`() =
        runTest(testCoroutineDispatcher) {
            // given a sync dispatcher configured to use native sync status
            val syncDispatcher = makeSyncDispatcher(
                syncConfig = syncConfig.copy(useNativeSyncStatus = true),
    ...
    @Test
    fun `when using native sync status, after a network error sync result, the observers go back to idle state`() =
        runTest(testCoroutineDispatcher) {
            // given a sync dispatcher configured to use native sync status
            val syncDispatcher = makeSyncDispatcher(
                syncConfig = syncConfig.copy(useNativeSyncStatus = true),
    ...
    /**
     * This test demonstrates the bug in https://bugzilla.mozilla.org/show_bug.cgi?id=2041554
     * Will be removed when [SyncConfig.useNativeSyncStatus] is removed
     */
    @Test
    fun `bug - when using work manager sync status, after a backed off sync, the observers never go back back to idle state`() =
        runTest(testCoroutineDispatcher) {
            // given a sync dispatcher configured to use work manager sync status
            val syncDispatcher = makeSyncDispatcher(
                syncConfig = syncConfig.copy(useNativeSyncStatus = false),
            )

            // given that we set up observers and initialize
            WorkersLiveDataObserver.setDispatcher(dispatcher = syncDispatcher)
            WorkersLiveDataObserver.init(testContext)
            syncDispatcher.register(testSyncStatusObserver)
            syncDispatcher.initialize()
            ...
            // then the observer changes should get stuck at "started"
            assertEquals(
                listOf(TestSyncStatus.STARTED),
                testSyncStatusObserver.syncStatus,
            )
        }
    ...
    /**
     * Helper function to make sure the work meets all the constraints and delays
     * and get executed immediately within the test lifecycle
     */
    private fun executeWork(workName: String) {
        val workInfos = WorkManager.getInstance(testContext)
    ...
    private fun makeSyncDispatcher(
    ...
    companion object {
        ...
        private val BackedOffSyncResult = SyncResult(
        ...
        private val NetworkErrorSyncResult = SyncResult(
        ...
        private val SuccessfulSyncResult = SyncResult(
        ...
        private val AuthenticatedAccount = object : TestOAuthAccount() {
    ...
    private class TestRustSyncManager : RustSyncManager() {
        ...
        override suspend fun executeSync(params: SyncParams): SyncResult {
            // in the real prod implementation, the call is blocking, and would typically give
            // enough time for collectors to collect the latest changes
            // however, in test, because this function returns immediately, we want to simulate
            // a suspension point, so that the collectors get a chance to observe the changed
            // counter value
            yield()
